...
class MockCatalogLocalDataSource extends Mock
    implements CatalogLocalDataSource {}
...
void main() {
  ...
  group('MockCatalogRepository', () {
    test(
      'getHomeFeed returns deterministic feed and caches it when online',
      () async {
        ...
        final googleOne = feed.featuredProducts.firstWhere(
          (p) => p.id == 'prod_google_one',
        );
    ...
    test(
      'getHomeFeed returns cached feed when offline and marks it stale',
      () async {
    ...
    test(
      'getHomeFeed returns OfflineFailure when offline and no cache exists',
      () async {
        when(() => mockConnectivity.isConnected).thenAnswer((_) async => false);
        when(
          () => mockLocalDataSource.getCachedHomeFeed(),
